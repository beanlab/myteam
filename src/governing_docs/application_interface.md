...
the CLI. It also scaffolds version-aware maintenance instructions so agents can surface upgrade
guidance for older `.myteam/` trees.
...
- Creates built-in maintenance skills under `.myteam/myteam/`.
...
- The generated `.myteam/myteam/` subtree includes maintenance skills that can print migration guidance and changelog notes for newer releases.
...
- The generated root role can direct the caller to built-in maintenance skills for upgrade details instead of invoking a dedicated migration command.
