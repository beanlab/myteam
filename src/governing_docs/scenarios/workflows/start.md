...
- agent2 calls `myteam result`
    - `myteam result` connects to other_workflow.py `run_agent` and sends result
    - other_workflow.py `run_agent` closes agent2 session and returns result
- other_workflow.py does additional work and eventually concludes
    - supervisor notes conclusion and records stdout/err
- supervisor resumes workflow.py process tree including agent1 session and
  `myteam start` command and routes other_workflow.py stdout/err to the output of `myteam start other_workflow.py`
    - agent1 sees output from other_workflow.py and continues its work
- agent1 calls `myteam result` and concludes
    - `myteam result` connects to workflow.py `run_agent` and reports result
    - workflow.py `run_agent` closes agent1 session and returns result
...
From `agent1`'s perspective, its call to `myteam start` eventually completes and returns the output from
`other_workflow.py`.
...
`myteam start` requests that a workflow process be started. This is a different socket than that used by `run_agent` for result reporting.

The supervisor has only one process `start`ed at a time. When a new process is requested via a nested
`myteam start`, it suspends the active process tree and starts the new one. When the new one finishes, it resumes the prior process tree recursively. The output from the
...
- listening on a socket for nested workflow requests;
...
- storing workflow outputs by request id;
...
The inner
`myteam start` connects to the existing supervisor socket, requests a nested workflow, waits until that workflow completes, prints the workflows output and stderr, and exits with an appropriate status code.
...
MYTEAM_WORKFLOW_ID=<current-workflow-process-id>
```
...
The inner `myteam start` client/shim should use polling to retrieve the nested child output.
...
supervisor stores output under workflow ID
...
client/shim prints output
client/shim exits
```

The client/shim may be suspended as part of the parent session after it receives the workflow ID. That is expected. When the parent session is resumed, the client/shim continues polling, receives the stored output, prints it, and exits. The parent agent then observes its
...
`myteam start` client/shim prints the child's stdout and stderr (to its respective stdout/err).

For small and medium results, the JSONL-RPC payload will contain the result directly:
...
For large results or artifacts, the child workflow can write data to a file and print instructions containing the path:

```json
{
  "exit-code": 0,
  "stdout": "See results in scratch/output.txt",
  "stderr": ""
}
```
