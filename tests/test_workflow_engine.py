...
def test_run_workflow_logs_completed_step_result_as_yaml(monkeypatch):
    messages: list[str] = []
    ...
    def fake_execute_step(
        ...
        *,
        ...
        return StepResult(
            ...
            status="completed",
            output={"value": step_name},
            resolved_input={"source": "authored"},
            agent_name=default_agent,
    ...
    assert result.status == "completed"
    assert messages[0] == "Starting step 'draft'"
    assert messages[1] == "Completed step 'draft'"
    assert messages[2] == (
        "agent_name: codex\n"
        "error_message: null\n"
        "error_type: null\n"
